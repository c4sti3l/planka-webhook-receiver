...
function extractEventData(payload) {
    return {
        eventType: payload.type || payload.action || payload.event || (payload.data && payload.data.type) || 'unknown', user: {
            name: payload.user?.name || payload.included?.users?.[0]?.name || 'Unknown', email: payload.user?.email || payload.included?.users?.[0]?.email || ''
        }, card: {
            name: payload.item?.name || payload.included?.cards?.[0]?.name || '', id: payload.item?.id || payload.included?.cards?.[0]?.id || ''
        }, board: {
            name: payload.included?.boards?.[0]?.name || '', id: payload.included?.boards?.[0]?.id || ''
        }, list: {
            name: payload.included?.lists?.[0]?.name || '', id: payload.included?.lists?.[0]?.id || ''
        }, comment: payload.item?.text || '', attachment: payload.item?.name || ''
    };
}
